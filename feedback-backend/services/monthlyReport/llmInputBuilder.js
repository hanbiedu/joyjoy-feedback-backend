...
function buildLLMInput(monthlyInputs) {
  ...
  5) 본 수업은 100% 1:1 수업이다.
- "다른 친구", "친구들", "또래", "함께", "모두", "누가" 등 다인 대상 표현 절대 금지
- 사회성(social)은 '교사-아이' 상호작용만으로 서술한다.
  (예: 교사의 제안에 시선/몸으로 반응, 도움 요청 신호, 전환 상황에서 안정, 교사와 번갈아 하기 등)
  ...
  - α 도메인은 변화 근거가 매우 명확할 때만 1개 추가(최대 3개)
  
  [도메인 값 제한]
  - domain 값은 반드시 아래 4개 중 하나만 사용한다:
    sensory, cognition, motor, social
  - domain에 “탐색 활동/선택 과정” 같은 제목형 단어를 쓰지 않는다.
  
  [문장 형식(가장 중요)]
  - summary는 정확히 2문장이다.
  - 두 문장은 반드시 자연스럽게 이어지는 ‘한 문단 호흡’으로 쓴다.
    * 1문장: 핵심 관찰 장면 + 반복/지속(있다면)
    * 2문장: 그 장면이 이어지는 방식(전환/유지/선택/조절 중 1개)
  - 문장 2개를 ‘나열’하지 말고, 연결 표현을 사용한다:
    “~였고, ~했으며 / ~이어졌고, ~나타났습니다 / ~반응이 있었으며, ~흐름이 이어졌습니다”
  
  [내용 제한]
  - 발달 평가 금지: “발달/능력/수준/문제 해결 능력/정서적 자기조절” 같은 표현 금지
  - 1:1 수업 고정: “친구/또래/함께/모두/다른 아이” 표현 절대 금지
    * social은 반드시 ‘교사-아이’ 상호작용으로만 서술한다(선생님/교사 포함)
  
  [좋은 예시(형식만 참고)]
  - sensory: “재료를 손으로 확인하는 장면이 반복되었고, 촉감 변화를 따라 손의 움직임을 이어갔습니다. 잠깐 멈춘 뒤에도 다시 돌아와 탐색을 이어가는 흐름이 관찰되었습니다.”
  - social: “선생님의 제안에 시선과 몸으로 반응하는 장면이 이어졌고, 반응이 오가며 상호작용이 지속되었습니다. 활동 전환에서도 선생님의 안내를 따라 흐름을 유지하는 모습이 관찰되었습니다.”
